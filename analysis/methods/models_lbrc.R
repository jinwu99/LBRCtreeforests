...
.logrank_trafo4 <- function(x2,eps=1e-7,max_iter=30){
  if (sum(x2[, 3] == 1) == 0) {
    result <- x2[,3]
  } else {
    delta <- x2[,3]
    ...
    logS_z_order <- logS_z_order[match(sort(Z), t)]
...
# - tune.metric: Character. Out-of-bag tuning criterion for selecting mtry
#   when mtry = NULL; one of c("brier", "cindex"). "brier" selects mtry by
#   minimizing the out-of-bag integrated Brier score, whereas "cindex" selects
#   mtry by maximizing the out-of-bag C-index based on predicted risk scores.
#
# Details:
...
lbrccif <- function(formula, data, id,
  ...
  if (is.null(mtry)){
    tune.metric <- match.arg(tune.metric)
