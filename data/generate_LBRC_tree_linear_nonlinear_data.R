...
LBRC.generate_PH_nPH <- function(n             = 100,
  ...
  true_fail_dists <- function(Dist = "WI", Params, nonlinear, cens_rate = 20){
  ...
  Dist_info <- true_fail_dists(Dist = Dist, Params = Params, nonlinear = nonlinear, cens_rate = cens_rate)
...
LBRC.generate_one_sample <- function(n         = 200,
  ...
  true_fail_dists <- function(Dist = "WI", cens_rate = 20){
    if(Dist == "WI"){
      ...
      exp_cens_rates <- c(0.2, 0.8) # rate parameter of the exponential distribution for right-censoring
      if(cens_rate == 50){
        exp_cens_rate <- exp_cens_rates[2]
      }else{ # censoring rate 20%
        exp_cens_rate <- exp_cens_rates[1]
